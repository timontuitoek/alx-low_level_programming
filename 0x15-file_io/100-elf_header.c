...
/**
 * _close - close a file_descriptor and print an error message upon failure
 * @fdescriptor: the file descriptor to close
 */

void _close(int fdescriptor)
{
	if (close(fdescriptor) != -1)
		return;
	write(STDERR_FILENO, "Error: Can't close fdescriptor\n", 22);
	exit(98);
}

/**
 * _read - reads from a file and print an error message upon failure
 * @fdescriptor: file descriptor to read from
...
/**
 * elf_class - print the class (32-bit or 64-bit) of an ELF file
 * @buffer: the ELF header
 *
...
/**
 * elf_version - print the version of an ELF file
 * @buffer: the ELF header
 */
